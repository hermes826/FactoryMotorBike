...
function normalizeBikePayload(payload = {}) {
  ...
  if (output.kilometers !== undefined && output.kilometers !== null) output.kilometers = Number(output.kilometers);
...
export async function addBikeImage(req, res) {
  const bike = await Bike.findById(req.params.id);
  if (!bike) return res.status(404).json({ detail: 'Moto no encontrada' });
  if (!req.file) return res.status(400).json({ detail: 'No se ha recibido ninguna imagen' });
